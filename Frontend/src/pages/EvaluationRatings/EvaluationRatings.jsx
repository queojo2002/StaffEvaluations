...
import NewAndUpdateEvaluationRatings from "./NewAndUpdateEvaluationRatings";

import { getAllCategoryRating, getAllUnit, getAllUserType, removeCategoryRating, removeRangeUserType } from "~/apis";
...
const EvaluationRatings = () => {
  ...
  const fetchApiGetAll = async () => {
    setLoading(true);
    const data1 = await getAllCategoryRating();
  ...
  const handleButtonDelete = () => {
    confirm({
      ...
      onOk: async () => {
        const res = await removeCategoryRating(selectedRowKeys);
        if (res.isSuccess) {
  ...
  return (
    <Spin spinning={loading}>
      <Modal
        style={{ top: 20 }}
        title="Thêm mới thang điểm đánh giá"
        open={isOpenModalAdd}
        onOk={() => {}}
        onCancel={() => {
