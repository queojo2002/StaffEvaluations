...
import NewAndUpdateEvaluationProsCons from "./NewAndUpdateEvaluationProsCons";

import { getAllCategoryCriteria, getAllCategoryProsCons, removeCategoryCriteria, removeCategoryProsCons } from "~/apis";
import Breadcrumbs from "~/components/Breadcrumbs";

const EvaluationProsCons = () => {
  ...
  const fetchApiGetAll = async () => {
    setLoading(true);
    const data1 = await getAllCategoryProsCons();
  ...
  const handleButtonDelete = () => {
    confirm({
      ...
      onOk: async () => {
        const res = await removeCategoryProsCons(selectedRowKeys);
        if (res.isSuccess) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "isPros",
      width: 160,
      render: (isPros, record) => (
        <div style={{ whiteSpace: "pre-wrap", width: "100%" }}>
          {isPros ? <Text type="success">Ưu điểm</Text> : <Text type="danger">Nhược điểm</Text>}
        </div>
      )
    },
    {
  ...
  return (
    <Spin spinning={loading}>
      <Modal
        title="Thêm mới ưu hoặc nhược điểm đánh giá"
        open={isOpenModalAdd}
        onOk={() => {}}
        onCancel={() => {
        ...
        width={800}
      >
        <NewAndUpdateEvaluationProsCons refetchApi={fetchApiGetAll} closeModal={setIsOpenModalAdd} id={id} />
      </Modal>
...
export default EvaluationProsCons;
